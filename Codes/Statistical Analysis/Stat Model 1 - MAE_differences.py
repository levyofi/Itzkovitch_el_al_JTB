...
data = pd.read_csv("/home/ofir/Dropbox/eclipse workspace/lab/Alon/M1_MAE.csv")
# Prepare categorical indices for 'Map'
maps = pd.Categorical(data['Map']).codes
# Prepare stages, treating 'M1_ME' as the baseline
stage_effect = pd.Categorical(data['Stage'], categories=['M1_MAE', 'M2_MAE', 'M3_MAE'], ordered=True).codes - 1
...
print(summary)
summary.to_csv("Alon_MAE_betas.csv")

# Plot the posterior trace
#az.plot_trace(trace)
trace.to_netcdf("Alon_MAE_betas.nc")
...
pp_samples.to_netcdf("Alon_MAE_betas_pp.nc")
